...
class Settings:
	...
	def populate(self, options):
		...
		self.Interface = (
			options.Interface if options.Interface else utils.FindFirstInterface()
		)
		...
		if self.IPv6:
			try:
				...
				if not self.Bind_To6:
					raise ValueError
				self.IP_Pton6 = socket.inet_pton(socket.AF_INET6, str(self.Bind_To6))
